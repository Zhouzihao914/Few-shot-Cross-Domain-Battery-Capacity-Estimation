...
class Coreg():
    ...
    def add_data(self, data_dir, X, y,signal = 'default'):
        """
    ...
    def run_trials(self, selected_index, num_train=100, trials=10, verbose=False,):
        """
        ...
        while self.trial < self.num_trials:
            ...
            self.train(selected_index, verbose=verbose,
                store_results=True)
            #print('Finished trial {}: {:0.2f}s elapsed\n'.format(
            #   self.trial + 1, time() - t0))
            self.trial += 1

    def train(self, selected_index,
        ...
        self._split_battery_data(selected_index)
    ...
    def _split_data(self, random_state=-1, num_labeled=100, num_test=1000):
        ...
        self.X_unlabeled = self.X_shuffled[num_labeled:test_end]
        self.y_unlabeled = self.y_shuffled[num_labeled:test_end]
    ...
    def _split_battery_data(self, choose_index):

        remain_index = list(set(range(len(self.y))) - set(choose_index))
        self.selected_index = choose_index

        self.X_labeled = self.X[choose_index]
        self.y_labeled = self.y[choose_index]
        self.X_test = self.X[remain_index]
        self.y_test = self.y[remain_index]

        self.X_unlabeled = self.X[remain_index]
        self.y_unlabeled = self.y[remain_index]
